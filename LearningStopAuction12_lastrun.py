...
# -*- coding: utf-8 -*-
"""
This experiment was created using PsychoPy2 Experiment Builder (v1.82.01), Fri Apr 10 17:38:10 2015
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath=u'/Users/patrickbissett/OneDrive/Poldrack/TrainedInhibition/PsychoPy/LearningStopAuction12.psyexp',
...
pracTrials = data.TrialHandler(nReps=0, method='fullRandom', 
    extraInfo=expInfo, originPath=u'/Users/patrickbissett/OneDrive/Poldrack/TrainedInhibition/PsychoPy/LearningStopAuction12.psyexp',
...
for thisStopBlock in StopBlocks:
    ...
    # set up handler to look after randomisation of conditions etc
    StopTrials = data.TrialHandler(nReps=5, method='fullRandom', 
        extraInfo=expInfo, originPath=u'/Users/patrickbissett/OneDrive/Poldrack/TrainedInhibition/PsychoPy/LearningStopAuction12.psyexp',
        trialList=data.importConditions('trialtypeStop.xlsx'),
    ...
    # completed 5 repeats of 'StopTrials'
...
for thisAuctionBlock in auctionBlocks:
    ...
    for thisAuctionTrial in auctionTrials:
        ...
        while continueRoutine and routineTimer.getTime() > 0:
            ...
            # *R* updates
            if t >= 1.5 and R.status == NOT_STARTED:
                # keep track of start time/frame for later
                R.tStart = t  # underestimates by a little under one frame
                R.frameNStart = frameN  # exact frame index
                R.setAutoDraw(True)
            if R.status == STARTED and t >= (1.5 + (5-win.monitorFramePeriod*0.75)): #most of one frame period left
                R.setAutoDraw(False)
            ...
            # *T* updates
            if t >= 1.5 and T.status == NOT_STARTED:
                # keep track of start time/frame for later
                T.tStart = t  # underestimates by a little under one frame
                T.frameNStart = frameN  # exact frame index
                T.setAutoDraw(True)
            if T.status == STARTED and t >= (1.5 + (5.0-win.monitorFramePeriod*0.75)): #most of one frame period left
                T.setAutoDraw(False)
            
            # *Y* updates
            if t >= 1.5 and Y.status == NOT_STARTED:
                # keep track of start time/frame for later
                Y.tStart = t  # underestimates by a little under one frame
                Y.frameNStart = frameN  # exact frame index
                Y.setAutoDraw(True)
            if Y.status == STARTED and t >= (1.5 + (5.0-win.monitorFramePeriod*0.75)): #most of one frame period left
                Y.setAutoDraw(False)
            
            # *U* updates
            if t >= 1.5 and U.status == NOT_STARTED:
                # keep track of start time/frame for later
                U.tStart = t  # underestimates by a little under one frame
                U.frameNStart = frameN  # exact frame index
                U.setAutoDraw(True)
            if U.status == STARTED and t >= (1.5 + (5.0-win.monitorFramePeriod*0.75)): #most of one frame period left
                U.setAutoDraw(False)
            
            # *I* updates
            if t >= 1.5 and I.status == NOT_STARTED:
                # keep track of start time/frame for later
                I.tStart = t  # underestimates by a little under one frame
                I.frameNStart = frameN  # exact frame index
                I.setAutoDraw(True)
            if I.status == STARTED and t >= (1.5 + (5.0-win.monitorFramePeriod*0.75)): #most of one frame period left
                I.setAutoDraw(False)
            
            # *O* updates
            if t >= 1.5 and O.status == NOT_STARTED:
                # keep track of start time/frame for later
                O.tStart = t  # underestimates by a little under one frame
                O.frameNStart = frameN  # exact frame index
                O.setAutoDraw(True)
            if O.status == STARTED and t >= (1.5 + (5.0-win.monitorFramePeriod*0.75)): #most of one frame period left
                O.setAutoDraw(False)
            
            # *RAmount* updates
            if t >= 1.5 and RAmount.status == NOT_STARTED:
                # keep track of start time/frame for later
            ...
            if RAmount.status == STARTED and t >= (1.5 + (5.0-win.monitorFramePeriod*0.75)): #most of one frame period left
                RAmount.setAutoDraw(False)
            
            # *TAmount* updates
            if t >= 1.5 and TAmount.status == NOT_STARTED:
                # keep track of start time/frame for later
            ...
            if TAmount.status == STARTED and t >= (1.5 + (5.0-win.monitorFramePeriod*0.75)): #most of one frame period left
                TAmount.setAutoDraw(False)
            
            # *YAmount* updates
            if t >= 1.5 and YAmount.status == NOT_STARTED:
                # keep track of start time/frame for later
            ...
            if YAmount.status == STARTED and t >= (1.5 + (5.0-win.monitorFramePeriod*0.75)): #most of one frame period left
