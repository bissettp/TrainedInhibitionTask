...
# -*- coding: utf-8 -*-
"""
This experiment was created using PsychoPy2 Experiment Builder (v1.81.03), Tue Mar  3 12:45:37 2015
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath=u'/Users/patrickbissett/OneDrive/Poldrack/TrainedInhibition/PsychoPy/LearningTask4.psyexp',
...
StopOrGoAndAmountLoop = data.TrialHandler(nReps=8, method='random', 
    extraInfo=expInfo, originPath=u'/Users/patrickbissett/OneDrive/Poldrack/TrainedInhibition/PsychoPy/LearningTask4.psyexp',
    trialList=data.importConditions(u'StopOrGoAndAmount.xlsx'),
    seed=None, name='StopOrGoAndAmountLoop')
thisExp.addLoop(StopOrGoAndAmountLoop)  # add the loop to the experiment
thisStopOrGoAndAmountLoop = StopOrGoAndAmountLoop.trialList[0]  # so we can initialise stimuli with some values
# abbreviate parameter names if possible (e.g. rgb=thisStopOrGoAndAmountLoop.rgb)
if thisStopOrGoAndAmountLoop != None:
    for paramName in thisStopOrGoAndAmountLoop.keys():
        exec(paramName + '= thisStopOrGoAndAmountLoop.' + paramName)

for thisStopOrGoAndAmountLoop in StopOrGoAndAmountLoop:
    currentLoop = StopOrGoAndAmountLoop
    # abbreviate parameter names if possible (e.g. rgb = thisStopOrGoAndAmountLoop.rgb)
    if thisStopOrGoAndAmountLoop != None:
        for paramName in thisStopOrGoAndAmountLoop.keys():
            exec(paramName + '= thisStopOrGoAndAmountLoop.' + paramName)
    ...
    # set up handler to look after randomisation of conditions etc
    ShapeLoop = data.TrialHandler(nReps=8, method='random', 
        extraInfo=expInfo, originPath=u'/Users/patrickbissett/OneDrive/Poldrack/TrainedInhibition/PsychoPy/LearningTask4.psyexp',
    ...
    # completed 8 repeats of 'ShapeLoop'
...
# completed 8 repeats of 'StopOrGoAndAmountLoop'

# get names of stimulus parameters
if StopOrGoAndAmountLoop.trialList in ([], [None], None):  params = []
else:  params = StopOrGoAndAmountLoop.trialList[0].keys()
# save data for this loop
StopOrGoAndAmountLoop.saveAsExcel(filename + '.xlsx', sheetName='StopOrGoAndAmountLoop',
    stimOut=params,
    dataOut=['n','all_mean','all_std', 'all_raw'])
