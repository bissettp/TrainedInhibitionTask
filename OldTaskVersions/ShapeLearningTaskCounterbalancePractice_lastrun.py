...
# -*- coding: utf-8 -*-
"""
This experiment was created using PsychoPy2 Experiment Builder (v1.82.01), Wed Apr  1 12:30:37 2015
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath=u'/Users/patrickbissett/OneDrive/Poldrack/TrainedInhibition/PsychoPy/ShapeLearningTaskCounterbalancePractice.psyexp',
...
# Initialize components for Routine "GetReadyForMainLearning"
GetReadyForMainLearningClock = core.Clock()
...
instrStopText = visual.TextStim(win=win, ori=0, name='instrStopText',
    text=u'A shape stimulus will appear on the left or right side of the screen\n\nIf it appears on the left, press Z\n\nIf it appears on the right, press M\n\nIf you hear a tone, do not press anything on that trial\n\nResponding quickly to the location of the shape and withholding your response when you hear a tone are equally important. ',    font=u'Arial',
    pos=[0, 0], height=0.1, wrapWidth=None,
...
pracTrials = data.TrialHandler(nReps=2.5, method='random', 
    extraInfo=expInfo, originPath=u'/Users/patrickbissett/OneDrive/Poldrack/TrainedInhibition/PsychoPy/ShapeLearningTaskCounterbalancePractice.psyexp',
    trialList=data.importConditions(u'TrialtypesLearning.xlsx'),
...
# completed 2.5 repeats of 'pracTrials'
...
#------Prepare to start Routine "GetReadyForMainLearning"-------
t = 0
GetReadyForMainLearningClock.reset()  # clock 
frameN = -1
# update component parameters for each repeat
# keep track of which components have finished
GetReadyForMainLearningComponents = []
for thisComponent in GetReadyForMainLearningComponents:
    if hasattr(thisComponent, 'status'):
        thisComponent.status = NOT_STARTED

#-------Start Routine "GetReadyForMainLearning"-------
continueRoutine = True
while continueRoutine:
    # get current time
    t = GetReadyForMainLearningClock.getTime()
    frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
    # update/draw components on each frame
...
#-------Ending Routine "GetReadyForMainLearning"-------
for thisComponent in GetReadyForMainLearningComponents:
    if hasattr(thisComponent, "setAutoDraw"):
        thisComponent.setAutoDraw(False)
# the Routine "GetReadyForMainLearning" was not non-slip safe, so reset the non-slip timer
routineTimer.reset()
